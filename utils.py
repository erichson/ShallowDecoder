...
def summary_pod_noisy(X_small_noisy, Xsmall, Xsmall_test, sensors, sensors_test, Xmean, sensor_locations, alpha=0, case='naive_pod'):
    from sklearn import linear_model
    ...
    # Get dimensions
    n_sensors = len(sensor_locations)       
    tt, _, mt = X_small_noisy.shape
    
    # Get Data from GPU to CPU
    Xsmall_temp = X_small_noisy.data.numpy().reshape(tt, mt) 
    sensors_temp = sensors.cpu().data.numpy().reshape(tt, n_sensors).T
